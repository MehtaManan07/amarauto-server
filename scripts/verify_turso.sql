...
SELECT '==== 1. ROW COUNTS (expect: users 21, parties 690, stages 4, products 883, raw_materials 232, operations 400 [276 mapped + 124 unmapped], bom_lines 2085, product_components 0, batches 0) ====' AS section;
...
UNION ALL SELECT 'operations_unmapped (product_id NULL)', COUNT(*) FROM operations WHERE product_id IS NULL
UNION ALL SELECT 'bom_lines', COUNT(*) FROM bom_lines
UNION ALL SELECT 'product_components', COUNT(*) FROM product_components
UNION ALL SELECT 'batches', COUNT(*) FROM batches;

SELECT '==== 2. MIGRATION VERSION (expect: merge_unmapped_ops_0003) ====' AS section;
SELECT version_num FROM alembic_version;

SELECT '==== 2b. ALL TABLES (expect: 14 app tables + alembic_version = 15; pending_operations dropped) ====' AS section;
...
SELECT '==== 5. OPERATIONS (expect: total 400, mapped 276, with_stage ~395, with_rate ~395) ====' AS section;
SELECT COUNT(*) total, COUNT(product_id) mapped, COUNT(stage_id) with_stage, COUNT(rate) with_rate FROM operations;

SELECT '==== 6. UNMAPPED OPERATIONS (product_id NULL — expect 124; top by hint = Cushport parts) ====' AS section;
SELECT COUNT(*) unmapped FROM operations WHERE product_id IS NULL;
SELECT product_hint, COUNT(*) n FROM operations WHERE product_id IS NULL
GROUP BY product_hint ORDER BY n DESC LIMIT 8;
...
SELECT '==== 9. BOM stage split + coverage (expect: cutting 644 / stitching 565 / finishing 876; 56 with BOM, 68 with ops — DISTINCT product_id ignores the 124 unmapped/NULL ops) ====' AS section;
...
SELECT
 ...
 (SELECT COUNT(*) FROM operations o LEFT JOIN products p ON o.product_id=p.id
    WHERE o.product_id IS NOT NULL AND p.id IS NULL) AS ops_orphan_product;  -- NULL product_id = intentionally unmapped, NOT an orphan
